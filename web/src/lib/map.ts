  ...
  map = new maplibregl.Map({
    ...
    style: {
      ...
      sources: {
        "carto-dark": {
          type: "raster",
          tiles: ["https://a.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png?key=cb1_2i6g_1_acb8c049139549a1b56e3610"],
          tileSize: 256,
          attribution:
...
export async function updateMarker() {
  ...
  const positions = new Float32Array([normalizeLongitude(config.lon), config.lat])
  const oceanTile = preloader.getTileIndicesForOcean(positions, 0);
  preloader.preloadTiles(currentDate, oceanTile);
  simulation.landmaskPromise = simulation.proteus?.init_landmask(normalizeLongitude(config.lon), config.lat) ?? null;
