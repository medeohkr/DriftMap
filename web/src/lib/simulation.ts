...
export function validateSimulation() {
  ...
  if (isNaN(config.particleCount) || config.particleCount <= 0 || config.particleCount > 50000)
...
export async function simulationStep(version: number) {
  if (!simulation.simulationRunning || version !== simulation.simulationVersion || !simulation.proteus) return;
  ...
  try {
    ...
    if (simulation.stepCount % config.stepsPerDay === 0) {
      ...
      preloader.preloadFutureSteps(todayDateInt, simulation.proteus.get_positions(), 2, 0);
      // Clean old tiles
    ...
    if (
      ...
      performance.now() - visualization.lastGridUpdate > visualization.gridUpdateInterval
    ) {
    ...
    if (simulation.stepCount < config.totalDays * config.stepsPerDay) {
      simulation.animationId = requestAnimationFrame(() => simulationStep(version));
    } else {
...
export async function resetSimulation() {
  ...
  if (map) {
    (map.getSource('concentration') as GeoJSONSource).setData({ type: 'FeatureCollection', features: [] });
    (map.getSource('particles-unstranded') as GeoJSONSource).setData({ type: 'FeatureCollection', features: [] });
    (map.getSource('particles-stranded') as GeoJSONSource).setData({ type: 'FeatureCollection', features: [] });
  }
...
export function updateStats() {
  stats.stranded = simulation.proteus?.stranded_fraction()?.toFixed(1) ?? '0.0';
  stats.emulsified = simulation.proteus?.mass_weighted_emulsification()?.toFixed(1) ?? '0.0';
  stats.evaporated = simulation.proteus?.mass_weighted_evaporation()?.toFixed(1) ?? '0.0';
  stats.totalMass = simulation.proteus?.total_floating_mass_tons()?.toFixed(1) ?? '0.0';
}
