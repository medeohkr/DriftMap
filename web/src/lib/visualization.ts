import { map } from './map';
import { config, simulation, timeline, visualization } from './stores.svelte';
...
export function tonsPerKm2ToTonsPerCell(value: number) {
  ...
  const cellAreaKm2 = visualization.gridSize * kmPerDegreeLon * (visualization.gridSize * kmPerDegreeLat);
...
export function updateParticleVisualization() {
  ...
  (map.getSource('particles-unstranded') as GeoJSONSource).setData(geojsonUnstranded);
  (map.getSource('particles-stranded') as GeoJSONSource).setData(geojsonStranded);
...
export function buildHeatmap() {
  ...
  visualization.heatmap = new HeatmapGenerator(
    lonMin - padding, lonMax + padding * 2,
    visualization.boundingBox[2] - padding, visualization.boundingBox[3] + padding,
...
export function updateBoundingBox() {
    visualization.boundingBox = simulation.proteus?.get_particle_bounding_box() ?? new Float32Array;
}