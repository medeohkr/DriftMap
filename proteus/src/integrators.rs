...
pub fn euler_step(
    ...
    lat: f32,
    dt: f32,
    velocity_fn: impl Fn(f32, f32) -> (f32, f32),
) -> (f32, f32) {
...
pub fn midpoint_step(
    ...
    // First half step
    let (u1, v1) = velocity_fn(lon, lat);
    let lon_mid = lon + 0.5 * dt * u1;
    let lat_mid = lat + 0.5 * dt * v1;
    
    // Velocity at midpoint
    let (u_mid, v_mid) = velocity_fn(lon_mid, lat_mid);
...
pub fn rk4_step(
    ...
    let (u2, v2) = velocity_fn(lon2, lat2);
    
    // k3
    let lon3 = lon + 0.5 * dt * u2;
    let lat3 = lat + 0.5 * dt * v2;
    let (u3, v3) = velocity_fn(lon3, lat3);
    
    // k4
    let lon4 = lon + dt * u3;
    let lat4 = lat + dt * v3;
    let (u4, v4) = velocity_fn(lon4, lat4);
