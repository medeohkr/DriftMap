...
use super::{Particles, ParticleView, find_depth_indices, bilerp, normalize_lon, meters_per_degree_lat, meters_per_degree_lon};
use half::f16;
use thiserror::Error;
...
impl DataLoader {
    ...
    pub async fn load_by_date(&mut self, date: usize, tiles: &HashSet<TileKey>) -> Result<(), LoaderError> {
        ...
        for (key, group) in groups {
            if let Some(tile) = self.cache.get(&key) {
                ...
                for (idx, lon, lat, depth) in group {
                    let (lon_cell, lat_cell) = self.get_cell_index(lon, lat, tile.n_lon, tile.n_lat, self.step, tile_min_lon, tile_min_lat);
                    ...
                    let (wlon_cell, wlat_cell) = self.get_cell_index(lon, lat, tile.n_lon_wind, tile.n_lat_wind, self.step_wind, tile_min_lon, tile_min_lat);
                    ...
                    let wind_u_m = bilerp(&tile.wind_u, wfrac_lon, wfrac_lat, widx, tile.n_lon_wind);
                    let wind_v_m = bilerp(&tile.wind_v, wfrac_lon, wfrac_lat, widx, tile.n_lon_wind);
                   
                    results[idx] = (meters_per_degree_lon(current_u_m, lat), meters_per_degree_lat(current_v_m), wind_u_m, wind_v_m);
                }
            }
        }
        ...
        for (key, group) in groups {
            if let Some(tile) = self.cache.get(&key) {
                ...
                for (idx, lon, lat, depth) in group {
                    ...
                    results[idx] = (meters_per_degree_lon(current_u_m, lat), meters_per_degree_lat(current_v_m))
                }
            }
        }
    ...
    pub fn get_wind_sst(
        ...
        for (i, lon, lat, _) in view.iter() {
            let lon = normalize_lon(lon);
            let key = self.get_tile_key(lon, lat, day);
            groups.entry(key).or_insert_with(Vec::new).push((i, lon, lat));
        }
    ...
    pub fn get_cell_index(&self, lon: f32, lat: f32, n_lon: usize, n_lat: usize, step: f32, tile_min_lon: f32, tile_min_lat: f32) -> (usize, usize) {
...
pub fn parse_tile_data(bytes: &[u8]) -> Result<TileData, String> {
    ...
    for _ in 0..n_steps {
        ...
        wind_u.extend(
            wind_u_f16.chunks_exact(2).map(|c| f16::from_bits(u16::from_le_bytes([c[0], c[1]])).to_f32())
        );
        
        let wind_v_f16 = &bytes[offset..offset + n_cells_wind * 2];
        offset += n_cells_wind * 2;
        wind_v.extend(
            wind_v_f16.chunks_exact(2).map(|c| f16::from_bits(u16::from_le_bytes([c[0], c[1]])).to_f32())
        );
